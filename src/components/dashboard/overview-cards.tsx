...
function OverviewCardsContent({ userId }: OverviewCardsProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="rounded-2xl bg-[#1F2937] dark:bg-white border-0 shadow-lg overflow-hidden">
        <CardContent className="p-6">
          <div className="flex justify-between items-start">
            {/* Monthly Total */}
            <div className="space-y-1">
              <p className="text-sm font-medium text-white/60 dark:text-[#1F2937]/60 uppercase tracking-wide font-sans">Monthly</p>
              <p className="text-3xl font-bold text-white dark:text-[#1F2937] tracking-tight font-sans">
                {formatCurrency(monthlyTotal, currency)}
              </p>
              <p className="text-xs text-white/40 dark:text-[#1F2937]/40 font-sans">{analytics?.totalSubscriptions || 0} subscriptions</p>
            </div>

            {/* Yearly Total */}
            <div className="space-y-1 text-right">
              <p className="text-sm font-medium text-white/60 dark:text-[#1F2937]/60 uppercase tracking-wide font-sans">Yearly</p>
              <p className="text-3xl font-bold text-white dark:text-[#1F2937] tracking-tight font-sans">
                {formatCurrency(yearlyTotal, currency)}
              </p>
              <div className="flex items-center justify-end gap-1 text-xs text-white/40 dark:text-[#1F2937]/40 font-sans">
          ...
          {isPremium && budget ? (
            <div className="mt-6 space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-xs font-medium text-white/60 dark:text-[#1F2937]/60 font-sans">Budget</span>
              ...
              <Progress
                ...
                className="h-2 bg-white/10 dark:bg-[#1F2937]/10"
            ...
            <Link href="/dashboard/budget" className="block mt-6">
              <div className="flex items-center justify-between p-3 rounded-xl bg-white/5 dark:bg-[#1F2937]/5 hover:bg-white/10 dark:hover:bg-[#1F2937]/10 transition-colors">
                <span className="text-sm text-white/70 dark:text-[#1F2937]/70 font-sans">Set a monthly budget</span>
                <span className="text-xs text-white/40 dark:text-[#1F2937]/40 font-sans">Tap to configure</span>
              </div>
            </Link>
          ) : (
            ...
            <Link href="/dashboard/upgrade" className="block mt-6">
              <div className="flex items-center justify-between p-3 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 transition-colors border border-amber-500/20">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-amber-200 dark:text-amber-600 font-medium font-sans">Unlock budget tracking</span>
                </div>
                <span className="text-xs text-amber-400/70 dark:text-amber-500/70 font-sans">Upgrade to Plus</span>
              </div>
            </Link>
          )}
        </CardContent>
